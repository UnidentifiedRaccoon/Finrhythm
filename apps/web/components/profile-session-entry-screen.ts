  ...
  if (phase === "contact" && profileSessionToken) {
    return h<ProfileContactScreenProps>(ProfileContactScreen, {
      ...
      sessionReadyNotice: {
        ...
        body:
          "Контактный профиль открыт в этой вкладке после принятия документов и безопасной передачи к N1. Сессионный секрет остаётся только в памяти компонента и не переносится через адрес."
      }
    });
  }
