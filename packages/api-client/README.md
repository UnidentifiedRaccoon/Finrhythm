...
Этот пакет намеренно покрывает только текущие публичные/admin DTO, admin code-status client helper и legal document acceptance client helper. Он не является универсальным генератором всего будущего API.
